...
public class ServiceHebergement implements IService<Hebergement> {
    ...
    public Hebergement getHebergementById(int id) {
        ...
        return null; // Return null if no hebergement is found
    }
    public Hebergement getById(int id) {
        for (Hebergement h : getAll()) {  // ✅ Use getAll() instead of non-existing getAllHebergements()
            if (h.getId_hebergement() == id) {
                return h;
            }
        }
